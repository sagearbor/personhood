...
## Current state — last updated 2026-09-08 (overnight round-4 session)
...
| **Fuzzy-extractor-selfie anchor** | `src/methods/fuzzy-extractor-selfie/` | **Airdrop-test anchor (checklist #10a).** Strength 70, anchor, $0.00, no vendor. From-scratch pure-Go fuzzy-extractor (Juels-Wattenberg fuzzy commitment over a repetition code — see the package's extractor.go doc comment) rather than an FFI wrap of the OpenLine Rust prototype (see the decision note in that doc comment). Server-side `Accumulator` does a Sybil-dedup membership scan (fuzzy-extractor `Rep` against every stored helper-data record — no raw biometric ever stored). `src/server/did.go`'s `NullifierBindingForBiometricCommitment` binds the issued credential's `nullifierBinding` to this method's biometric commitment (not the regenerable holder device keypair) whenever it's the anchor — closing the "mint a new keypair, get a new nullifier" loophole for OpenLine's UBI-claim / vote use cases. Registers when `FUZZY_EXTRACTOR_ENABLED=1` (no vendor credential to gate on otherwise). 20 unit tests + 6 server-integration tests + a real-server e2e test, all `-race` green. |
